...
public class AnimatedImage implements Image, HasBorder {
	...
	static abstract class Element {
		...
		abstract void render(int time, Img next, int x, int y, int width,
				int height, AnimatedImage ai, AnimationState as);
	}
	...
	static class Img extends Element {
		...
		Img(int duration, Image image, Color tintColor, float zoomX,
				float zoomY, float zoomCenterX, float zoomCenterY) {
			if (duration < 0) {
		...
		void render(int time, Img next, int x, int y, int width, int height,
				AnimatedImage ai, AnimationState as) {
			...
			ai.renderer.pushGlobalTintColor(rr * ai.r, gg * ai.g, bb * ai.b, aa
					* ai.a);
			try {
				int zWidth = (int) (width * zx);
				int zHeight = (int) (height * zy);
				image.draw(as, x + (int) ((width - zWidth) * cx), y
						+ (int) ((height - zHeight) * cy), zWidth, zHeight);
			} finally {
	...
	AnimatedImage(Renderer renderer, Element root, String timeSource,
			Border border, Color tintColor, int frozenTime) {
