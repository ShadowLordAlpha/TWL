...
public class Mouse {
	...
	public static boolean isCreated() {
		if(mbe == null) {
			passMBE = GLFW.glfwSetMouseButtonCallback(GLFW.glfwGetCurrentContext(), (mbe = new MouseButtenEvent()));
		}
		
		if(mse == null) {
			passMSE = GLFW.glfwSetScrollCallback(GLFW.glfwGetCurrentContext(), (mse = new MouseScrollEvent()));
		}
		
		if(mme == null) {
			passMME = GLFW.glfwSetCursorPosCallback(GLFW.glfwGetCurrentContext(), (mme = new MouseMoveEvent()));
		}
	...
	private static class MouseButtenEvent extends GLFWMouseButtonCallback {
		...
		ByteBuffer xpos = BufferUtils.createByteBuffer(4);
		ByteBuffer ypos = BufferUtils.createByteBuffer(4);
		
		@Override
		public void invoke(long window, int button, int action, int mods) {
			GLFW.glfwGetCursorPos(window, xpos, ypos);
			if(action == GLFW.GLFW_PRESS || action == GLFW.GLFW_REPEAT) {
				eventStack.addElement(new Event(xpos.getInt(0), ypos.getInt(0), 0, button, true));
			} else {
				eventStack.addElement(new Event(xpos.getInt(0), ypos.getInt(0), 0, button, false));
			}
	...
	private static class MouseScrollEvent extends GLFWScrollCallback {
		...
		public void invoke(long window, double xoffset, double yoffset) {
			GLFW.glfwGetCursorPos(window, xpos, ypos);
			eventStack.addElement(new Event(xpos.getInt(0), xpos.getInt(0), (int) Math.round(xoffset), -1, false));
